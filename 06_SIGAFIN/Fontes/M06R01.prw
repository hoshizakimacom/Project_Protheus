...
aCmp := FWSX3Util():GetAllFields( "SC6" , .T. )
	...
	For nCont := 1 to Len(aCmp[nCont])


			cValid := UPPER(aCmp[]) + UPPER(GetSx3Cache(aCmp[nCont],"X3_VLDUSER"))
...
//		While SX3->(!EOF()).And. SX3->X3_ARQUIVO == 'SC5'
			cValid := UPPER(GetSx3Cache(aCmp[nCont],"X3_VALID")) + UPPER(etSx3Cache(aCmp[nCont],"X3_VLDUSER"))
