...
User Function M06A11(cNumBor)
	...
	If !lAuto 
		...
		cFiltro		+= "E1_FILIAL=='"+xFilial("SE1")+"'.And.E1_SALDO > 0 .And."
		cFiltro		+= "E1_PREFIXO>='" + MV_PAR01 + "'.And.E1_PREFIXO<='" + MV_PAR02 + "'.And."
		cFiltro		+= "E1_NUM>='" + MV_PAR03 + "'.And.E1_NUM<='" + MV_PAR04 + "'.And."
		cFiltro		+= "E1_PARCELA>='" + MV_PAR05 + "'.And.E1_PARCELA<='" + MV_PAR06 + "'.And."
		cFiltro		+= "E1_PORTADO>='" + MV_PAR07 + "'.And.E1_PORTADO<='" + MV_PAR08 + "'.And."
		cFiltro		+= "E1_CLIENTE>='" + MV_PAR09 + "'.And.E1_CLIENTE<='" + MV_PAR10 + "'.And."
		cFiltro		+= "E1_LOJA>='" + MV_PAR11 + "'.And.E1_LOJA<='"+MV_PAR12+"'.And."
		cFiltro		+= "DTOS(E1_EMISSAO)>='"+DTOS(mv_par13)+"'.and.DTOS(E1_EMISSAO)<='"+DTOS(mv_par14)+"'.And."
		cFiltro		+= 'DTOS(E1_VENCREA)>="'+DTOS(mv_par15)+'".and.DTOS(E1_VENCREA)<="'+DTOS(mv_par16)+'".And.'
		cFiltro		+= 'DTOS(E1_XRENEG)>="'+DTOS(mv_par30)+'".and.DTOS(E1_XRENEG)<="'+DTOS(mv_par31)+'".And.'
		cFiltro		+= "E1_NUMBOR>='" + MV_PAR17 + "'.And.E1_NUMBOR<='" + MV_PAR18 + "'.And."
		cFiltro		+= "!(E1_TIPO$MVABATIM).And."
		cFiltro		+= "E1_PEDIDO>='" + MV_PAR25 + "'.And.E1_PEDIDO<='" + MV_PAR26 + "'.And."
		cFiltro		+= "E1_TIPO$'NF |BOL'"   //.And."

//		cFiltro		+= "E1_NUMBOR<>'      '.And."   // Teste
//		cFiltro		+= "!EMPTY(E1_NUMBCO)"          // Teste
		...
		@ 001,001 TO 400,700 DIALOG oDlg TITLE "Seleção de Titulos"
			...
			oMrkBrowse:SetFilterDefault(cFiltro)
