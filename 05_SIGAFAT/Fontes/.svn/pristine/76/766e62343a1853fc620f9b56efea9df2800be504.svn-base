...
//+-------------------------------------------------------------------------------------------------
User Function M05D07U()
    Local   _aSays          := {}
    Local   _aButton        := {}
    Local   _cTitulo        := FunName()

    AADD(_aSays,OemToAnsi("Esta rotina tem como objetivo alterar complementos de produtos (B5) a partir "))
    AADD(_aSays,OemToAnsi("de um arquivo CSV. "))
...
Static Function MD05Proc()
    Local _oDlg         := Nil
    Local _cTitle       := 'Alteração de Complemento de Produtos'
    Local _oArqOri      := Nil
    Local _oArqLog      := Nil
...
Static Function MD05Ok()
    ...
    If _lValid
        ...
        For _nItem := 1 To Len(_aItens)
            FWMsgRun(, {||MD05Exec(_aField,_aItens[_nItem],@_cLog,@_nErr,@_nInc) },,I18N('Alterando produto #1 de #2 ...',{++_nReg,_nTotal}))
        Next
...
Static Function MD05Log(_cArqLog,_cMsg,_cLog,_nErr,_nInc)
    ...
    _cLog := CRLF + CRLF + I18N('Produtos alterado: #1',{_nInc})  +  CRLF + I18N('Produtos não alterados (erro): #1',{_nErr}) + CRLF + CRLF + _cLog
...
Static Function MD05Exec(_aField,_aItem,_cLog,_nErr,_nInc)
    ...
    Local aComp         := {}
        ...
        If ( nPos := AScan(aProd,{|x| AllTrim(x[1]) == 'B5_COD'}) ) > 0
            _cCod       := aProd[nPos][2]
        EndIf
...
Static Function MD05GetLog(_cCod,_cMsgLog,_cLog)
	...
	Local _nTam	:= ( TamSX3("B1_COD")[1] - Len(_cCod) ) 
	
    _cLog += CRLF
    _cLog += ' | Código:  '      + _cCod + Space(_nTam) + '  ' 
...
BeginTran()
	...
	If DbSeek(xFilial("SB5") + cCodigo)
	    ...
	    oModel := NIL
	Else
		_cMsgLog    := 'Produto ' + cCodigo + ' não encontrado na tabela SB1.'
	Endif	    	

EndTran()

//    MODELO ANTIGO - Sem MVC 
//
//    lMsErroAuto := .F.
//
//    MSExecAuto({|x,y| Mata180(x,y)},aProd,4)
//        
//    If lMsErroAuto
//        ++_nErr
//        _cMsgLog    := MD05GetErr()
