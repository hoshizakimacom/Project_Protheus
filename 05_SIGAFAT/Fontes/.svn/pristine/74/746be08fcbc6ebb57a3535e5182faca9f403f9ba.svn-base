...
//+-------------------------------------------------------------------------------------------------
//  Rotina de importação de titulos a receber
//+-------------------------------------------------------------------------------------------------
User Function M05D05I()
    Local   _aSays          := {}
    Local   _aButton        := {}
    Local   _cTitulo        := FunName()

    AADD(_aSays,OemToAnsi("Esta rotina tem como objetivo importar titulos a receber a partir de um arquivo CSV."))
...
Static Function MD05Proc()
    Local _oDlg         := Nil
    Local _cTitle       := 'Importação Titulos a Receber'
    Local _oArqOri      := Nil
    Local _oArqLog      := Nil
...
Static Function MD05ArqOri()
    _cArqOri    := cGetFile("CSV | *.csv","Selecione arquivo de titulos a receber",,"",.T.,GETF_NETWORKDRIVE+GETF_LOCALHARD+GETF_LOCALFLOPPY )
Return
...
Static Function MD05Ok()
    ...
    If _lValid
        ...
        For _nItem := 1 To Len(_aItens)
            FWMsgRun(, {||MD05Exec(_aField,_aItens[_nItem],@_cLog,@_nErr,@_nInc) },,I18N('Importanto títulos #1 de #2 ...',{++_nReg,_nTotal}))
        Next
...
Static Function MD05Log(_cArqLog,_cMsg,_cLog,_nErr,_nInc)
    ...
    _cLog := CRLF + CRLF + I18N('Titulos Incluídos: #1',{_nInc})  +  CRLF + I18N('Títulos não incluídos (erro): #1',{_nErr}) + CRLF + CRLF + _cLog
...
Static Function MD05Exec(_aField,_aItem,_cLog,_nErr,_nInc)
    ...
    Local aReg         := {}
        ...
        aReg   := M05DTitulo(_aField,_aItem)

        If ( nPos := AScan(aReg,{|x| AllTrim(x[1]) == 'E1_PREFIXO'}) ) > 0
            cPrefixo       := aReg[nPos][2]
        EndIf

        If ( nPos := AScan(aReg,{|x| AllTrim(x[1]) == 'E1_NUM'}) ) > 0
            cNum       := aReg[nPos][2]
        EndIf

        If ( nPos := AScan(aReg,{|x| AllTrim(x[1]) == 'E1_TIPO'}) ) > 0
            cTipo       := aReg[nPos][2]
        EndIf

        If ( nPos := AScan(aReg,{|x| AllTrim(x[1]) == 'E1_PEDIDO'}) ) > 0
            cPedido       := aReg[nPos][2]
        EndIf


        MD05Put(aReg,@_cMsgLog,@_nErr,@_nInc)

        MD05GetLog(cPrefixo,cNum,cTipo,cPedido,_cMsgLog,@_cLog)
...
//+-------------------------------------------------------------------------------------------------
Static Function MD05GetLog(cPrefixo,cNum,cTipo,cPedido,_cMsgLog,_cLog)
    _cLog += CRLF
    _cLog += ' | Prefixo: '      + cPrefixo
    _cLog += ' | Numero: '        + cNum
    _cLog += ' | Tipo: '      + cTipo
    _cLog += ' | Tipo: '      + cPedido
...
//+-------------------------------------------------------------------------------------------------
Static Function M05DTitulo(aField,aItem)
    Local aRet      := {}
    Local nX        := 0

    For nX := 1 To Len(aField)

        If SubStr(aField[nX][1],1,2) == 'E1'
            Do Case
            Case aField[nX][2] == 'N'
...
Static Function M05DComp(aField,aItem)
    ...
    aRet := FWVetByDic(aRet,'SE1')
Return AClone(aRet)

//+-------------------------------------------------------------------------------------------------
Static Function MD05Put(aReg,_cMsgLog,_nErr,_nInc)
