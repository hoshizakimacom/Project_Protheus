...
//+-------------------------------------------------------------------------------------------------
//  Rotina de importação de produtos a partir de arquivo CSV
//+-------------------------------------------------------------------------------------------------
User Function M05D03I()
    Local   _aSays          := {}
    Local   _aButton        := {}
    Local   _cTitulo        := FunName()

    AADD(_aSays,OemToAnsi("Esta rotina tem como objetivo importar produtos a partir de um arquivo CSV."))
...
Static Function MD05Proc()
    Local _oDlg         := Nil
    Local _cTitle       := 'Importação Produtos'
    Local _oArqOri      := Nil
    Local _oArqLog      := Nil
...
Static Function MD05Ok()
    ...
    If _lValid
        ...
        For _nItem := 1 To Len(_aItens)
            FWMsgRun(, {||MD05Exec(_aField,_aItens[_nItem],@_cLog,@_nErr,@_nInc) },,I18N('Importanto produto #1 de #2 ...',{++_nReg,_nTotal}))
        Next
...
Static Function MD05Log(_cArqLog,_cMsg,_cLog,_nErr,_nInc)
    ...
    _cLog := CRLF + CRLF + I18N('Produtos Incluídos: #1',{_nInc})  +  CRLF + I18N('Produtos não incluídos (erro): #1',{_nErr}) + CRLF + CRLF + _cLog
