            ...
            For _nX:= 1 to (_cAlias)->C9_QTDLIB
                If !alltrim(_cCfop) $ "5923|6923" //ADD POR CLAUDIO AMBROSINI EM 270922 CHAMADO #1278
                    If !ZAB->(MsSeek(xFilial("ZAB")+(_cAlias)->C9_PEDIDO+(_cAlias)->C9_ITEM))
                        _lRet := .F.
                        MsgStop("O produto :  " + ALLtrim((_cAlias)->C9_PRODUTO) +  " - Item :  " + ALLtrim((_cAlias)->C9_ITEM) + "  não possui etiqueta de identificação. Acione a expedição ! OBS: FONTE M05A26","ATENCAO")
                        Exit
                    Endif
