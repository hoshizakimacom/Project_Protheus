...
//+-------------------------------------------------------------------------------------------------
//  Rotina de importação de pedidos a partir de arquivo CSV
//+-------------------------------------------------------------------------------------------------
User Function M05D04I()
    Local   _aSays          := {}
    Local   _aButton        := {}
    Local   _cTitulo        := FunName()

    AADD(_aSays,OemToAnsi("Esta rotina tem como objetivo importar pedidos a partir de um arquivo CSV."))
...
Static Function MD05Proc()
    Local _oDlg         := Nil
    Local _cTitle       := 'Importação Pedidos'
    Local _oArqOri      := Nil
    Local _oArqLog      := Nil
...
Static Function MD05ArqOri()
    _cArqOri    := cGetFile("CSV | *.csv","Selecione arquivo de Pedidos",,"",.T.,GETF_NETWORKDRIVE+GETF_LOCALHARD+GETF_LOCALFLOPPY )
Return
...
Static Function MD05Ok()
    ...
    If _lValid
        ...
        For _nItem := 1 To Len(_aItens)
            FWMsgRun(, {||MD05Exec(_aField,_aItens[_nItem],@_cLog,@_nErr,@_nInc) },,I18N('Importanto pedido #1 de #2 ...',{++_nReg,_nTotal}))
        Next
...
Static Function MD05Log(_cArqLog,_cMsg,_cLog,_nErr,_nInc)
    ...
    _cLog := CRLF + CRLF + I18N('Pedidos Incluídos: #1',{_nInc})  +  CRLF + I18N('Pedidos não incluídos (erro): #1',{_nErr}) + CRLF + CRLF + _cLog
...
Static Function MD05Exec(_aField,_aItem,_cLog,_nErr,_nInc)
    ...
    Local aVet2         := {}
        ...
        aVet1   := M05DVet1(_aField,_aItem)
        aVet2   := M05DVet2(_aField,_aItem)

        If ( nPos := AScan(aVet1,{|x| AllTrim(x[1]) == 'C5_NUM'}) ) > 0
            _cPedido       := aVet1[nPos][2]
        EndIf

        MD05Put(aVet1,aVet2,@_cMsgLog,@_nErr,@_nInc)

        MD05GetLog(_cPedido,_cMsgLog,@_cLog)
...
//+-------------------------------------------------------------------------------------------------
Static Function MD05GetLog(_cPedido,_cMsgLog,_cLog)
    _cLog += CRLF
    _cLog += ' | Pedido: '      + _cPedido
    _cLog += ' | STATUS: '      + AllTrim(_cMsgLog) + ' |'
Return

//+-------------------------------------------------------------------------------------------------
Static Function M05DVet1(aField,aItem)
    Local aRet      := {}
    Local nX        := 0

    For nX := 1 To Len(aField)

        If SubStr(aField[nX][1],1,2) == 'C5'
            Do Case
            Case aField[nX][2] == 'N'
            ...
            Case aField[nX][2] == 'M'
                AAAdd(aRet ,{aField[nX][1]   ,aItem[nX]                          ,Nil})
            EndCase
       EndIf
    Next

    aRet := FWVetByDic(aRet,'SC5')
Return AClone(aRet)

//+-------------------------------------------------------------------------------------------------
Static Function M05DVet2(aField,aItem)
    Local aRet      := {}
    Local nX        := 0

    For nX := 1 To Len(aField)

        If SubStr(aField[nX][1],1,2) == 'C6'
            Do Case
            Case aField[nX][2] == 'N'
                AAdd(aRet ,{aField[nX][1]   ,Val(aItem[nX])                     ,Nil})
            Case aField[nX][2] == 'C'
    ...
    aRet := FWVetByDic(aRet,'SC6')
Return AClone(aRet)

//+-------------------------------------------------------------------------------------------------
Static Function MD05Put(aVet1,aVet2,_cMsgLog,_nErr,_nInc)

    lMsErroAuto := .F.

    MSExecAuto({|x,y,z|Mata410(x,y,z)},aVet1,{aVet2},3)
