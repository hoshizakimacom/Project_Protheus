...
//+---------------------------------------------------------------------------------------------------------
//|	Rotina respons�vel pela cria��o dos titulos de recebimento antecipado com prefixo PVA
...
User Function M05A08()
	...
	//+------------------------------------
	// Valida informa��es do pedido/itens
	//+------------------------------------
...
//+--------------------------------------------------------------------------------------------
//|	Rotina respons�vel pela inclus�o do titulo a receber
//+--------------------------------------------------------------------------------------------
...
//	EndIf
	...
	MsExecAuto( { |x,y| FINA040(x,y)} , _aVet, 3)  // 3 - Inclusao, 4 - Altera��o, 5 - Exclus�o
...
//+--------------------------------------------------------------------------------------------
//|	Rotina respons�vel pela defini��o dos valores e datas das parcelas a serem geradas
//+--------------------------------------------------------------------------------------------
Static Function MA08Ok(_nRecnoSC5,_nRecAnt,_cCondPag,_cRAPre,_cNaturez)
	...
	Begin Transaction
		...
		For _nX := 1 To Len(_aDuplic)
			...
			// Posiciona SC5 - na grava��o da segunda parcela ele est� desposicionado  
	...
	If _lErro
		...
		MsgInfo(I18N('Erro ao incluir recebimento antecipado: ' + CRLF + CRLF + ' #1',{_cErro}),'Aten��o')
	Else
		
		MsUnlockAll()
		Aviso('Aten��o','Recebimento(s) gerados(s): ' + CRLF + CRLF + _cMsg,{'OK'},3)
	EndIf
	End Transaction
Return

//+--------------------------------------------------------------------------------------------
//|	Rotina respons�vel pela valida��o do total informado pelo usu�rio
//+--------------------------------------------------------------------------------------------
Static Function MA08ValidT(_nRecAnt,_nPedSaldo)
	...
	If _lRet .And. !(_lRet := _nRecAnt <= _nPedSaldo)
		MsgStop('Recebimento antecipado n�o pode ser maior que o valor do saldo total.')
	EndIf
Return _lRet

//+--------------------------------------------------------------------------------------------
//|	Rotina respons�vel pela valida��o da condi��o de pagamento informada pelo usu�rio
//+--------------------------------------------------------------------------------------------
...
//+--------------------------------------------------------------------------------------------
//|	Rotina respons�vel pela valida��o dos dados do pedido utilizados para gerar o reb. antecipado
//+--------------------------------------------------------------------------------------------
Static Function MA08Valid(_cNaturez)
	...
	If !_lRet
		MsgInfo('� obrigat�rio estar posicionado em um registro para gerar recebimento antecipado.','Aten��o')
	EndIf

	If _lRet .And. !(_lRet := !Empty(SC5->C5_CLIENTE))
		MsgInfo('Informe o cliente antes de incluir o recebimento antecipado.','Aten��o')
	EndIf
	...
	If _lRet
		...
		If !(_lRet := (SED->(DbSeek( xFilial('SED') + _cNaturez))))
			MsgInfo('Verifique Natureza informada no cadastro do cliente antes de incluir o recebimento antecipado.','Aten��o')
		EndIf
	EndIf
		...
		If SC6->(DbSeek( xFilial('SC6') + SC5->C5_NUM ))
			While SC6->(!EOF()) .And. SC6->(C6_FILIAL + C6_NUM ) ==  SC5->(C5_FILIAL + C5_NUM)
				...
				If !(_lRet := !(SC6->C6_CF $ _cCFBlq))
					MsgInfo(I18N('CFOP do item #1 n�o permite incluir recebimento antecipado.' ;
								+ CRLF + 'Par�metro #2.',{AllTrim(SC6->C6_ITEM),'MV_X06A002'}),'Aten��o')
					Exit
				EndIf
	...
	If _lRet .And. !(_lRet := (_nValor> 0))
		MsgInfo('N�o � poss�vel gerar recebimento antecipado para pedido sem Valor Bruto.' ;
					+ CRLF + 'Verifique quantidade dos itens.','Aten��o')
	EndIf
Return _lRet

//+--------------------------------------------------------------------------------------------
//|	Rotina respons�vel pela captura da mensagem de erro na execu��o autom�tica
//+--------------------------------------------------------------------------------------------
...
//+--------------------------------------------------------------------------------------------
//|	Rotina respons�vel pelo retorno do total de t�tulos de recebimento antecipados do pedido
//+--------------------------------------------------------------------------------------------
...
//+----------------------------------------------------------------------------------------
//|	Rotina respons�vel pelo c�lculo do total do pedido
//+----------------------------------------------------------------------------------------
