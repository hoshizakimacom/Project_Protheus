...
User Function M05A21()
    ...
    Define MsDialog _oDlg Title _cTitle Style DS_MODALFRAME  From _aSize[1],_aSize[2] To _aSize[3],_aSize[4] Pixel
        ...
        @032,160 Say  'Até:' Of _oDlg Pixel
		...
		@095,550 BUTTON "Estornar/Limpar Apontamento" SIZE 040, 015 PIXEL OF _oDlg ACTION (M05AEST(_cPedido,_cItem))
                                                                                         
        //@115,550 BUTTON "Limpar Apontamento" SIZE 040, 020 PIXEL OF _oDlg ACTION (M05AST(_cPedido,_cItem))
...
Local kx1:=0
    ...
    // *** Monta chave de pesquisa do usuário *** //
    nTam1 :=len(cUsuLimp)
    cTexto:=""
    nNumx1:=0
    cAtuali1:="N"
    for kx1:=1 to nTam1
        ...
        // *** Verifica se o usuário com acesso é o mesmo usuário que esta logado *** //
	...
	dbSelectArea("SC2")
	dbSetOrder(1)
    if alltrim(cUserLib) $cUsuEstorn  //cUsuEstorn
        ...
        Else
            Do Case
                ...
                Case Empty(_cEtapa) .And. ALLTRIM(UPPER(_cEtapaAtu)) == ALLTRIM(UPPER("Tec Com Lib"))
                    RecLock('SC6',.F.)
                    SC6->C6_XETAPA		:= " "
...
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|| Programa  |M05AEST2    |Autor |Valdemir Miranda          | Data | 09/02/2023||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|| Desc.     |Estorno Ordes de Produção                                        ||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
*/
Static Function M05AST2()
if alltrim(cUserLib) $cUsuLimp  // Usuário do João
