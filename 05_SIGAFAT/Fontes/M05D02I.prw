...
//+-------------------------------------------------------------------------------------------------
//  Rotina de importação de vendedores a partir de arquivo CSV
//+-------------------------------------------------------------------------------------------------
User Function M05D02I()
    Local   _aSays          := {}
    Local   _aButton        := {}
    Local   _cTitulo        := FunName()

    AADD(_aSays,OemToAnsi("Esta rotina tem como objetivo importar venderoes a partir de um arquivo CSV."))
...
Static Function MD05Proc()
    Local _oDlg         := Nil
    Local _cTitle       := 'Importação Vendedores'
    Local _oArqOri      := Nil
    Local _oArqLog      := Nil
...
Static Function MD05ArqOri()
    _cArqOri    := cGetFile("CSV | *.csv","Selecione arquivo de Vendedores",,"",.T.,GETF_NETWORKDRIVE+GETF_LOCALHARD+GETF_LOCALFLOPPY )
Return
...
Static Function MD05Ok()
    ...
    If _lValid
        ...
        For _nItem := 1 To Len(_aItens)
            FWMsgRun(, {||MD05Exec(_aField,_aItens[_nItem],@_cLog,@_nErr,@_nInc) },,I18N('Importanto vendedor #1 de #2 ...',{++_nReg,_nTotal}))
        Next
...
Static Function MD05Log(_cArqLog,_cMsg,_cLog,_nErr,_nInc)
    ...
    _cLog := CRLF + CRLF + I18N('Vendedores Incluídos: #1',{_nInc})  +  CRLF + I18N('Vendedores não incluídos (erro): #1',{_nErr}) + CRLF + CRLF + _cLog
...
Static Function MD05Exec(_aField,_aItem,_cLog,_nErr,_nInc)
    ...
    If ( nPos := AScan(aReg,{|x| AllTrim(x[1]) == 'A3_COD'}) ) > 0
        cCod       := aReg[nPos][2]
    EndIf

    If ( nPos := AScan(aReg,{|x| AllTrim(x[1]) == 'A3_NOME'}) ) > 0
        cNome       := aReg[nPos][2]
    EndIf

    MD05Put(aReg,@_cMsgLog,@_nErr,@_nInc)

    MD05GetLog(cCod,cNome,_cOrigem,_cMsgLog,@_cLog)
...
//+-------------------------------------------------------------------------------------------------
Static Function MD05GetLog(cCod,cNome,_cOrigem,_cMsgLog,_cLog)
    _cLog += CRLF
    _cLog += ' | Código: '      + cCod
    _cLog += ' | Nome: '        + (cNome)
...
Static Function MD05Put(aReg,_cMsgLog,_nErr,_nInc)
    ...
    BeginTran()
        ...
        MSExecAuto({|x,y| MATA040(x,y)},aReg,3)
