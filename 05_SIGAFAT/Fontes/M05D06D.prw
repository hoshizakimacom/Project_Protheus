...
/////////////////////////////////////////////////////////////
USER FUNCTION M05D06D()
LOCAL _ASAYS := {}
LOCAL _ABUTTON := {}
LOCAL _CTITULO := FUNNAME()

AADD(_ASAYS,OEMTOANSI("ESTA ROTINA TEM COMO OBJETIVO EXCLUIR CLIENTES A PARTIR DE UM ARQUIVO CSV."))
...
LOCAL _ODLG := NIL
LOCAL _CTITLE := "EXCLUSÃO CLIENTES"
LOCAL _OARQORI := NIL
LOCAL _OARQLOG := NIL
...
_CARQORI := CGETFILE("CSV | *.CSV","SELECIONE ARQUIVO DE CLIENTES",,"", .T. ,0+48+8)
RETURN 
...
IF _LVALID
    ...
    FOR _NITEM := 1 TO  LEN(_AITENS)
        FWMSGRUN(,{||MD05EXEC(_AFIELD,_AITENS[_NITEM],@_CLOG,@_NERR,@_NINC)},,I18N("EXCLUINDO CLIENTE #1 DE #2 ...",{++_NREG,_NTOTAL}))
    NEXT
...
_CLOG := CHR(13)+CHR(10) + CRLF+I18N("CLIENTES EXCLUÍDOS: #1",{_NINC}) + CRLF+I18N("CLIENTES NÃO EXCLUÍDOS (ERRO): #1",{_NERR}) + CRLF + CRLF+_CLOG
...
MD05GETLOG(CCOD,CLOJA,_CORIGEM,_CMSGLOG,@_CLOG)
...
// FONTE RECONSTRUIDO PELO TIME BSO ********************** //
/////////////////////////////////////////////////////////////
STATIC FUNCTION MD05GETLOG(CCOD,CLOJA,_CORIGEM,_CMSGLOG,_CLOG)
_CLOG += CHR(13)+CHR(10)
_CLOG += " | CÓDIGO: "+CCOD
_CLOG += " | LOJA: "+CLOJA
...
MSExecAuto({|a,b,c| CRMA980(a,b,c)},AREG,5,{}) //MSEXECAUTO({|X,Y|MATA030(X,Y)},AREG,5)
