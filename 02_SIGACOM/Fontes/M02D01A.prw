...
_CLOG := CHR(13)+CHR(10) + CRLF+I18N("FORNECEDORES ALTERADOS: #1",{_NINC}) + CRLF+I18N("FORNECEDORES NÃO ALTERADOS (ERRO): #1",{_NERR}) + CRLF + CRLF+_CLOG
...
FOR NX := 1 TO  LEN(AAUXAFIELD)
    ...
    IF!Empty(FWSX3Util():GetFieldType(CCAMPO))
        AADD(AFIELD,{CCAMPO,getsx33cache(CCAMPO,"X3_TIPO")})
    ELSE 
        LERRO :=  .T. 
        EXIT 
    ENDIF
