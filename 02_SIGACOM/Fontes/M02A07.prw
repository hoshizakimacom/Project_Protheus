...
//+--------------------------------------------------------------------------------------------------
//| Função que inclui anexo na Tabela de Preços
//  Chamado do ponto de entrada CM010MNU
//+--------------------------------------------------------------------------------------------------
User Function M02A07_A()

    Local _cMascara     := 'Todos os arquivos|*.*'
    Local _cTitulo      := 'Escolha o arquivo'
    Local _nMascpad     := 0
    Local _cDirOri      := 'C:\'
    Local _cDirDes      := AllTrim(GetMv('AM_02A07_A',.T.,''))
...
//+-------------------------------------------------------------------------------
User Function M02A07_c()
    Local _cDir     := AllTrim(GetMv('AM_02A07_A',.T.,''))

    If Empty(_cDir)
        Aviso('Atenção','É obrigatório informar o diretório de armazenamento no parâmetro AM_02A07_A.',{'OK'},3)
    Else
...
//AIA_FILIAL+AIA_CODFOR+AIA_LOJFOR+AIA_CODTAB