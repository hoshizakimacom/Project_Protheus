...
/*/
User Function M02R01( cAlias, nReg, nOpcx )
...
Local oSection1
Local oSection2
Private nTamCdProd:= TamSX3("C7_PRODUTO")[1]
...
//³ExpC5 : Descricao                                                       ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oReport:= TReport():New("M02R01",cTitle,"MTR110", {|oReport| ReportPrint(oReport,nReg,nOpcx)},STR0001+" "+STR0002)
oReport:SetPortrait()
...
TRCell():New(oSection2,"DESCPROD"   	,"   ",STR0097   	,/*Picture*/,60,/*lPixel*/, {|| cDescPro},,,,,,.T.)
...
If lAuto
	...
	mv_par05 := Eval({|| cCont:=U_ChkPerg(cUserId,"MTR110","05"),If(cCont == Nil,2,cCont) })
   	mv_par08 := Eval({|| cCont:=U_ChkPerg(cUserId,"MTR110","08"),If(cCont == Nil,C7_TIPO,cCont) })
	mv_par09 := Eval({|| cCont:=U_ChkPerg(cUserId,"MTR110","09"),If(cCont == Nil,1,cCont) })
  	mv_par10 := Eval({|| cCont:=U_ChkPerg(cUserId,"MTR110","10"),If(cCont == Nil,3,cCont) }) 
	mv_par11 := Eval({|| cCont:=U_ChkPerg(cUserId,"MTR110","11"),If(cCont == Nil,3,cCont) }) 
  	mv_par14 := Eval({|| cCont:=U_ChkPerg(cUserId,"MTR110","14"),If(cCont == Nil,1,cCont) }) 	
Else
	...
	//Incluida validação para os pedidos de compras por item do pedido  (IP/alçada)			
		...
		If !lNewAlc
			...
			If SC7->C7_TIPO == 1 .OR. SC7->C7_TIPO == 3
				...
				oReport:PrintText("As Faturas/Boletos para pagamentos aos fornecedores serão efetuados somente as SEXTAS-FEIRAS, respeitando o prazo minimo acordado em pedido de compra.",,050)
			Else
...
Local cCGC			:= ""
Public nRet		:= 0
Private cCident		:= IIF(len(SM0->M0_CIDENT)>20,Substr(SM0->M0_CIDENT,1,15),SM0->M0_CIDENT)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function M02R01R3(cAlias,nReg,nOpcx)
...
If nLastKey <> 27
	...
	If lAuto
		mv_par08 := Eval({|| cCont:=U_ChkPerg(cUserId,"MTR110","08"),If(cCont == Nil,SC7->C7_TIPO,cCont) })
	EndIf
...
If lAuto
	...
	mv_par05 := Eval({|| cCont:=ChkPerg(cUserId,"MTR110","05"),If(cCont == Nil,2,cCont) })
   	mv_par08 := Eval({|| cCont:=ChkPerg(cUserId,"MTR110","08"),If(cCont == Nil,C7_TIPO,cCont) })
...
If !lNewAlc
	...
	@ li,142 - nDifColCC PSAY "|"
	li++
	@ li,001 PSAY "As Faturas/Boletos para pagamentos aos fornecedores serão efetuados somente as SEXTAS-FEIRAS, respeitando o prazo minimo acordado em pedido de compra."
	@ li,142 - nDifColCC PSAY "|"
	li++
	@ li,001 PSAY "|"
	@ li,002 PSAY Replicate("-",limite)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function ChkPerg(cUserId,cGrupo,cSeq)
