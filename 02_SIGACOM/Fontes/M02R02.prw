...
//| Relatório PC
//+------------------------------------------------------------------------------------------------------------------------------------------------------
User Function M02R02(lEnvEmail)

DEFAULT lEnvEmail := .F.

FWMsgRun(, {|| U_M02R02A(lEnvEmail) },'Pedido de Compra','Gerando relatório...')

Return

//+------------------------------------------------------------------------------------------------------------------------------------------------------
User Function M02R02A(lEnvEmail)
    ...
    If !lEnvEmail
        cFileImp := 'PC' + SC7->C7_NUM + '_' + SubStr(DToS(Date()),7,2) + '_' + StrTran(Time(),":","")
        oPrinter := FWMSPrinter():New(cFileImp, IMP_PDF, .T./*_lAdjustToLegacy*/, /*cPathInServer*/, .T.)
    Else
        cFileImp := 'PC' + SC7->C7_NUM + '_' + DToS(Date())
    	cLocal   := "\SPOOL\"
...
//  oPrinter:Box(nRow,0100,nRow + nRowStep * 2,0200)                        // SQ
    oPrinter:Box(nRow,0180,nRow + nRowStep * 2.5,0320)                      // Item             oPrinter:Box(nRow,0180,nRow + nRowStep * 2.5,0403) 
//  oPrinter:Box(nRow,0400,nRow + nRowStep * 2,0700)                        // Código
    oPrinter:Box(nRow,0510,nRow + nRowStep * 2.5,1100)                      // Descrição        oPrinter:Box(nRow,0650,nRow + nRowStep * 2.5,1250) 
    oPrinter:Box(nRow,1240,nRow + nRowStep * 2.5,1400)                      // DESTINO DE USO
...
//  oPrinter:Box(nRow,2950,nRow + nRowStep * 2,3200)                        // VALOR TOTAL
    ...
    oPrinter:Say(nRow                ,0185                ,'ENTREGA'       ,oFont12B) // oPrinter:Say(nRow                ,0200                ,'DT.ENTREGA'    ,oFont12B) 
    oPrinter:Say(nRow                ,0330                ,'CÓDIGO'        ,oFont12B) // oPrinter:Say(nRow                ,0480                ,'CÓDIGO'        ,oFont12B)
    oPrinter:Say(nRow                ,0710                ,'DESCRIÇÃO'     ,oFont12B) // oPrinter:Say(nRow                ,0850                ,'DESCRIÇÃO'     ,oFont12B)
    oPrinter:Say(nRow                ,1120                ,'MEDIDA'        ,oFont12B) // oPrinter:Say(nRow                ,1260                ,'MEDIDA /'      ,oFont12B)
    oPrinter:Say(nRow                ,1250                ,'DESTINO'       ,oFont12B) // oPrinter:Say(nRow + nRowStep     ,1250                ,'DEST. USO'     ,oFont12B)
    oPrinter:Say(nRow + nRowStep     ,1260                ,'DE USO'        ,oFont12B)
    ...
    //oPrinter:Say(nRow                ,3680                ,'DESTINO DE USO'  ,oFont12B) //#4322
Return
...
Static Function MR02ImpItem(_cAlias,oPrinter,nRow,oFont9,oFont12B,nItem,nTotPed,nValFret,nValDesp,nValDesc,nValST)
    ...
    Local cDestUso      := Posicione("SB1",1,xFilial("SB1")+(_cAlias)->_PRODUTO,"B1_XDESUSO")   //#4322
...
// #4322 - Condição que avalia qual categoria deve ser preenchida no Pedido de Compras

   If cDestUso == "1"
      cDestUso := "Industr"      // Destino de Uso: 1-Industrialização; 
   ElseIf cDestUso == "2"
      cDestUso := "Revenda"      // Destino de Uso: 2-Revenda;
   ElseIf cDestUso == "3"
      cDestUso := "Consumo"      // Destino de Uso: 3-Consumo;
   ElseIf cDestUso == "4"
      cDestUso := "Serviço"      // Destino de Uso: 4-Serviço
   Else
      cDestUso := " "           // Destino de Uso: 
   EndIf

    oPrinter:Box(nRow,0100,nRow + (nRowStep * 1.25 * Len(aDesc)),3200)                     // SQ        
    oPrinter:Box(nRow,0180,nRow + (nRowStep * 1.25 * Len(aDesc)),0320)                     // Item      oPrinter:Box(nRow,0180,nRow + (nRowStep * 1.25 * Len(aDesc)),0403)
    oPrinter:Box(nRow,0510,nRow + (nRowStep * 1.25 * Len(aDesc)),1100)                     // Descrição oPrinter:Box(nRow,0650,nRow + (nRowStep * 1.25 * Len(aDesc)),1250)
    oPrinter:Box(nRow,1240,nRow + (nRowStep * 1.25 * Len(aDesc)),1400)                     // DESTINO DE USO
    ...
    //oPrinter:Box(nRow,3200,nRow + (nRowStep * 1.25 * Len(aDesc)),3650)                   // DESTINO DE USO #4322

    nRow += nRowStep

    oPrinter:Say(nRow - 0008   ,0120    ,SubStr(AllTrim((_cAlias)->_ITEM),1,15)                                                                                                 ,oFont9)    // ITEM
    oPrinter:Say(nRow - 0008   ,0190    ,Substr((_cAlias)->_DATPRF,7,2)+"/"+Substr((_cAlias)->_DATPRF,5,2)+"/"+Substr((_cAlias)->_DATPRF,1,4)                                   ,oFont9)    // DT.ENTREGA   oPrinter:Say(nRow - 0008   ,0225    ,Substr((_cAlias)->_DATPRF,7,2)+"/"+Substr((_cAlias)->_DATPRF,5,2)+"/"+Substr((_cAlias)->_DATPRF,1,4)            
    oPrinter:Say(nRow - 0008   ,0330    ,(_cAlias)->_PRODUTO                                                                                                                    ,oFont9)    // CÓDIGO       oPrinter:Say(nRow - 0008   ,0415    ,(_cAlias)->_PRODUTO                                                                                                                    ,oFont9)
    oPrinter:Say(nRow - 0008   ,1150    ,Posicione("SB1",1,xFilial("SB1")+(_cAlias)->_PRODUTO,"B1_UM")                                                                          ,oFont9)    // MEDIDA / DESTINO DE USO  oPrinter:Say(nRow - 0008   ,1267    ,Posicione("SB1",1,xFilial("SB1")+(_cAlias)->_PRODUTO,"B1_UM")+" / "+ cDestUso
    oPrinter:Say(nRow - 0008   ,1260    ,cDestUso                                                                                                                               ,oFont9)    //4322 
...
//    oPrinter:Say(nRow - 0008   ,3010    ,Transform(NoRound((_cAlias)->_TOTAL + ((_cAlias)->_FRETE + (_cAlias)->_DESPESA ) + (_cAlias)->_VALIPI,2) ,cPicVal)                     ,oFont12B)  // VALOR TOTAL
    oPrinter:Say(nRow - 0008   ,2780    ,Transform(NoRound((_cAlias)->_TOTAL + ((_cAlias)->_VALFRE + (_cAlias)->_DESPESA) + (_cAlias)->_VALIPI / (_cAlias)->_QTDCOM,2),cPicVal)  ,oFont9)    // VALOR UNIT C/ IMPOSTOS
    oPrinter:Say(nRow - 0008   ,3010    ,Transform(NoRound((_cAlias)->_TOTAL + ((_cAlias)->_VALFRE + (_cAlias)->_DESPESA ) + (_cAlias)->_VALIPI,2) ,cPicVal)                     ,oFont12B)  // VALOR TOTAL

    //Adiciona valor nas variáveis totalizadoras para uso no rodapé
//    nTotPed += 	((_cAlias)->_TOTAL + (_cAlias)->_VALIPI + (_cAlias)->_VALFRE + (_cAlias)->_DESPESA) 
    nTotPed += 	((_cAlias)->_TOTAL + (_cAlias)->_VALIPI + (_cAlias)->_DESPESA) 
    ...
    For nDesc := 1 to Len(aDesc)
        oPrinter:Say(nRow - 0008       ,0520           ,aDesc[nDesc]                                                                                                           ,oFont9)     // DESCRIÇÃO  oPrinter:Say(nRow - 0008       ,0660           ,aDesc[nDesc]                                                                                                           ,oFont9)
        nRow += nRowStep
    Next
...
Static Function MR02Cab2(oPrinter,oFont12,oFont18T,nRow,oFont14B)
    ...
    If lRet
        ...
        oPrinter:Say(nRow + 70            ,1300    , 'PEDIDO   DE   COMPRA'                     ,oFont18T)
...
Static Function MR02Cab4(oPrinter,oFont12,oFont14B,oFont18T,nRow)
    Local nRowStep    := 45
    Local cMoedaDesc  := GetMV("MV_MOEDA"+StrZero(SC7->C7_MOEDA,1))
    Local cTipoFrete  := SC7->C7_TPFRETE                                       //4525
    Private oBrush    := TBrush():New( , RGB( 240 ,240 ,240))

   If cTipoFrete == "C"
      cTipoFrete := "CIF"                           // Tipo de Frete: C-CIF; 
   ElseIf cTipoFrete == "F"
      cTipoFrete := "FOB"                           // Tipo de Frete: F-FOB;
   ElseIf cTipoFrete == "T"
      cTipoFrete := "Por Conta de Terceiros"        // Tipo de Frete: T-Por Conta de Terceiros;
   ElseIf cTipoFrete == "R"
      cTipoFrete := "Por Conta de Remetente"        // Tipo de Frete: T-Por Conta de Remetente
   ElseIf cTipoFrete == "D"
      cTipoFrete := "Por Conta de Destinatário"     // Tipo de Frete: D-Por Conta de Destinatário  
   ElseIf cTipoFrete == "S"
      cTipoFrete := "Sem Frete"                     // Tipo de Frete: Sem Frete
   Else
      cTipoFrete := "Não Informado"                 // Frete Não Informado
   EndIf
    ...
    oPrinter:Say(nRow += nRowStep     ,0100    , 'PEDIDO DE COMPRA: '     ,oFont12)
    oPrinter:Say(nRow                 ,0440    , SC7->C7_NUM  		      ,oFont14B)
    ...
    oPrinter:Say(nRow += nRowStep     ,0100    , 'TIPO DE FRETE: '                      ,oFont12)   //4525
    oPrinter:Say(nRow                 ,0440    , cTipoFrete                             ,oFont12)   //4525

    oPrinter:Say(nRow                 ,2700    , 'MOEDA: '                              ,oFont12)   //4525
    oPrinter:Say(nRow                 ,3050    , cMoedaDesc                             ,oFont12)   //4525

    oPrinter:Say(nRow += nRowStep     ,0100    ,'OBSERVAÇÕES'                           ,oFont12)   //4525
    oPrinter:Say(nRow                 ,0440    ,AllTrim(SC7->C7_OBS)                    ,oFont12)   //4525

    oPrinter:Say(nRow                 ,2700    ,'DATA EMBARQUE'                         ,oFont12)   //4525
    oPrinter:Say(nRow                 ,3050    ,Dtoc(SC7->C7_DT_EMB)                    ,oFont12)   //4525

    oPrinter:Say(nRow += nRowStep     ,0100    ,'COMPRADOR RESPONSÁVEL'                 ,oFont12)
    oPrinter:Say(nRow                 ,0550    ,IIF(SC7->C7_USER <> '000014',AllTrim(USRFULLNAME(SC7->C7_USER)),' '),oFont12)
...
//    Local cMoeda        := AllTrim(GetMv('MV_MOEDA' + cValToChar(SC5->C5_MOEDA),,''))
    ...
    oPrinter:Say(nRow + 1260                ,0100        , cPolitic     ,oFont14b)
     ...
     If nRow > 1600
        ...
        oPrinter:Say(nRow += nRowStep     ,0100    , I18N('Pedido de Compra #1 impresso em #2.',{SC7->C7_NUM,cData })  ,oFont12)
...
//    oPrinter:Say(nRow += nRowStep     ,2450    , SA3->A3_DDDTEL + ' ' + Transform(AllTrim(SA3->A3_TEL), "@R 9999-99999" ) ,oFont12)
    ...
    oPrinter:Say(nRow += nRowStep     ,0100    , 'NOTA: Só aceitaremos a mercadoria se na sua Nota Fiscal constar o numero de nosso Pedido de Compra'                                                      ,oFont12)
    oPrinter:Say(nRow += nRowStep     ,0100    , 'As faturas/Boletos para pagamentos aos fornecedores serão efetuados somente as SEXTAS-FEIRAS, respeitando o prazo mínimo acordado em pedido de compra.'  ,oFont12)        
