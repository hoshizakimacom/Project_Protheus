...
Else
	...
	Else
		If ZAB->(MSSeek(xFILIAL("ZAB")+cOP+cItem+cSequen))
				...
				*/
				...
				For _nX :=1 to _nQtdOP
					...
					Else
						...
						MsUnlock()
						*/
						PUTMV("AM_NUMSER",Alltrim(STR(_nSerial)))
						DBSelectArea("ZAB")
...
User Function M10EPrin(_cCodProd,_nQtd,_cNumSer,lCtrlImp,_nQtdImp)
	...
	//Local _dDtFab		:= dDataBase // Retirado pois aparentemente pega a data de emissão Posicione("SD3",18,xFilial("SD3")+ AllTrim(_cNumSer), "D3_EMISSAO") # retirado chamado 11372
	...
	_oPrinter:Say(_nRow + 67 ,650 , "Acesse o site: www.hoshizakimacom.com.br",_oFontP)
	...
	//_oPrinter:Say(_nRow + 320 , 70, "Data Fab: " + AllTrim(DTOC(_dDtFab)) ,_OFontP) # retirado chamado 11372
	_oPrinter:Say(_nRow + 360 , 70, "Numero de Serie: " ,_OFontP)
	...
	_oPrinter:QrCode(700,713,_cQRCode, 090) // #7976

	_oPrinter:Say(_nRow + 530 , 70, "ESPECIFICAÇÕES TÉCNICAS: " ,_OFontP)
	...
	If _cFamilia == "000001" 
		...
		If _cTpGas <> "4"
				...
				_oPrinter:Say(_nRow + 600 , 800, "Corrente: " 			+ _cCorrente + " A", _OFontP)
				_oPrinter:Say(_nRow + 630 , 800, "Tensão: " 			+ UPPER(_cTensao), _OFontP)
				_oPrinter:Say(_nRow + 660 , 800, "Frequência: " 		+ _cFreq, _OFontP)

		Else

				_oPrinter:Say(_nRow + 600 , 800, "Gás: " 				+ "Ciclopentano", _OFontP)
				_oPrinter:Say(_nRow + 600 , 70, "Fluído Refrigerante: " + _cTxFluido ,_OFontP)
				_oPrinter:Say(_nRow + 630 , 70, "Carga de Fluído: "     + _cVlrFluido + "g", _OFontP)
				_oPrinter:Say(_nRow + 660 , 70, "Potência: " 			+ _cPotencia, _OFontP)

				//_oPrinter:Say(_nRow + 580 , 450, "Potência degelo: " 	+ _cPdeGelo , _OFontP)
				_oPrinter:Say(_nRow + 630 , 450, "Grau de Proteção: IP" + _cGrProtecao, _OFontP)
				_oPrinter:Say(_nRow + 660 , 450, "Classe Climática: " 	+ _cClClima , _OFontP)
				_oPrinter:Say(_nRow + 600 , 450, "Corrente: " 			+ _cCorrente + " A", _OFontP)
...
User Function M10EPri1(_cCodProd,_nQtd,_cNumSer,lCtrlImp)
	...
	//Private _dDtFab		    := Posicione("SD3",18,xFilial("SD3")+ AllTrim(_cNumSer), "D3_EMISSAO") # retirado chamado 11372
	...
	_oPrinter:Say(_nRow += 40 ,650 , "CNPJ: 43.553.668/0001-79",_oFontP)
	...
	_oPrinter:Say(_nRow + 515 , 150, AllTrim(_cOP) ,_OFontP) //#7032
	...
	_oPrinter:Say(_nRow + 295 , 700, "BCode Serial: " ,_OFontP)
	...
	_oPrinter:Say(_nRow + 685 , 1000, "FGQ-008 Rev.01" ,_OFontP) /**#7666**/	
