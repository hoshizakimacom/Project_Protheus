...
//+---------------------------------------------------------------------------------
// Rotina responsavel pelo apontamento de etapa de produ��o
//+---------------------------------------------------------------------------------
User Function M10A01()
    ...
    //+--------------------------------------------------------
    // Busca etapa e local de transfer�ncia
    //  Caso grupo do usu�rio logado n�o possua etapa,
    //  n�o permite apontamento
    //+--------------------------------------------------------
    M10AGetEt()

    If Empty(_cEtapaDes)
        M10AMsg("Usu�rio sem permiss�o para acessar essa rotina.",'Aten��o',.F.)
    Else

        DEFINE MSDIALOG _oDlg FROM 0,0 TO 350,800 TITLE "Apontamento de Etapas de Produ��o" PIXEL
            ...
            _oTSayUsr           := TSay():New(020,020,{||"Usu�rio:"}    ,_oPnlAll,,_oTFont,.F.,.F.,.F.,.T.,0,,200,020,.F.,.T.,.F.,.F.,.F.,.F. )
            ...
            _oTSayCB            := TSay():New(105,020,{||"C�d. Barras:"},_oPnlAll,,_oTFont,.F.,.F.,.F.,.T.,0,,200,020,.F.,.T.,.F.,.F.,.F.,.F. )
    ...
    If _lValid
        BEGIN TRANSACTION //BeginTrans()
            ...
            If !Empty(ZA3->ZA3_LOCAL)
                //+--------------------------------------------------------
                // Cria saldo inicial se necess�rio
...
//                  If !_lErro
//                      _lErro := !M10APrD3T()      // cria Transfer�ncia
//                  EndIf
                EndIf
            EndIf
            ...
            //+--------------------------------------------------------
            //  Somente efetiva movimenta��o se n�o houver erro
            //+--------------------------------------------------------
            If _lErro
                DisarmTransaction()
            //Else
        ...
        If !_lErro
            M10AMsg(I18N('Apontamento de #1 realizado com sucesso!',{AllTrim(ZA3->ZA3_DESCRI)}),'Aten��o',.T.)
        EndIf
    EndIf
...
Static Function M10APrD3T()
    ...
    If SC2->C2_XLOCAL <> ZA3->ZA3_LOCAL
        ...
        AAdd(_aAux,{"D3_DESCRI",  _cDesc,				 Nil})  //  Descri��o
        AAdd(_aAux,{"D3_UM",	  _cUM,					 Nil})  //  Unid. Medida
        AAdd(_aAux,{"D3_LOCAL",   SC2->C2_XLOCAL,		 Nil})	//  Armazem origem
        AAdd(_aAux,{"D3_LOCALIZ", CriaVar('D3_LOCALIZ'), Nil})  //  Endere�o Origem
        AAdd(_aAux,{"D3_COD", 	  _cProd,				 Nil})  //  Produto Destino
        AAdd(_aAux,{"D3_DESCRI",  _cDesc, 				 Nil})  //  Descri��o
        AAdd(_aAux,{"D3_UM",	  _cUM,					 Nil})  //  Unid. Medida
        AAdd(_aAux,{"D3_LOCAL",	  ZA3->ZA3_LOCAL,		 Nil})	//  Armaz�m destino
        AAdd(_aAux,{"D3_LOCALIZ", CriaVar('D3_LOCALIZ'), Nil})  //  Endere�o Destino
        ...
        AAdd(_aAux,{"D3_QTSEGUM", CriaVar('D3_QTSEGUM'), Nil})  //  Qtde 2� UM
        ...
        AAdd(_aAux,{"D3_OBSERVA", CriaVar('D3_OBSERVA'), Nil})	//	Observa��o
        ...
        If lMsErroAuto
            _cErro  := M10AGetEr()
            M10AMsg('Erro ao incluir transfer�ncia:' + CRLF + _cErro,'Erro',.F.)
        EndIf
    EndIf
Return !lMsErroAuto
...
Static Function M10AGetOP()
    ...
    Else
        M10AMsg(I18N('Ordem de Produ��o #1 n�o encontrada.',{AllTrim(_cOP)}),'Aten��o',.F.)
        M10ASetVa(.F.)
    EndIf
Return _lOk
...
Static Function M10ASetSa()
    ...
    //+--------------------------------------------------------
    //  Verifica se existe saldo inicial no armaz�m origem
...
Static Function M10AVlEtp()
    ...
    //+--------------------------------------------------------
    //  Verifica se etapa j� foi apontada anteriormente
    //+--------------------------------------------------------
    If _lOk
        SC2->(DbGoTo(_nRecnoSC2))

        // Verifica se etapa j� apontada anteriormente
        ...
        If _nRecnoZA3 == 0
            M10AMsg('Etapa j� apontada para esta Ordem de Produ��o.','Aten��o',.F.)
            _lOk := .F.
        EndIf
    EndIf
Return _lOk
...
Static Function M10ASetC5()
    ...
    //+--------------------------------------------------------
    // Caso etapa seja ultima de produ��o, verifica se o pedido
    // n�o est� cancelado e atualiza seu status
    //+--------------------------------------------------------
    If ZA3->ZA3_ULTIMA == 'S'
        ...
        //+--------------------------------------------------------
        // Caso o pedido n�o esteja cancelado, verifica se todos os
        //  itens est�o com status 9 (em expedi��o)
        //+--------------------------------------------------------
        If !_lCancel
