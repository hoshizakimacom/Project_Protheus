...
Private aSB1Cod	:= TAMSX3("B1_COD")
Private aSB1Ite	:= TAMSX3("B1_CODITE")
Private aSizeVL	:= TamSX3("B2_VATU1")
Private aSizeLZ   := TamSX3("NNR_DESCRI")
Private cPictQT   := PesqPict("SB2","B2_QATU")
Private cPictVL   := PesqPict("SB2","B2_VATU1")
Private cPictLZ   := PesqPict("NNR","NNR_DESCRI")
...
TRCell():New(oSection,'B1_XFAMILI'  ,'SB1')
...
LOcal nX         := 0
Private nSoma      := nSoma2    := 0
Private nTotSoma   := nTotSoma2 := 0
Private nRegM0     := 0
Private nIndB1     := 0
Private nIndB2     := 0
Private nQtdProd   := 0
...
Private cDesc
Private lIsCient
Private nQtdBlq    := nQtdBlq2 := 0
Private nQuant     := 0.00
Private nQuant2    := 0.00
Private aFiliais   := {}
Private cNomArqB2  := ''
...
cGroupBy := "%"
If ! lVEIC
	cGroupBy += " SB2.B2_COD, SB1.B1_XFAMILI,SB1.B1_TIPO, SB1.B1_GRUPO"
Else	
...
BeginSql Alias cAliasTOP
	...
	,SB1.B1_TIPO TIPO, SB1.B1_XFAMILI FAMILIA, SB1.B1_GRUPO GRUPO, SB1.B1_DESC, SB1.B1_UM UM
	...
	While !oReport:Cancel() .And. !(cAliasTOP)->(Eof())
		...
		If (mv_par14 == 1 .Or. ( mv_par14 <> 1 .and. QtdComp(nQuant) <> QtdComp(0))) .And. ;
			...
			oSection:Cell("B1_XFAMILI"):SetValue(Posicione("ZA1",1,xFilial("ZA1")+(cAliasTOP)->FAMILIA,"ZA1_DESC"))
...
Local WnRel      := 'MATR240'
Private nTotQuebra := nTotQuebr2 := 0

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Variaveis tipo Local para SIGAVEI, SIGAPEC e SIGAOFI         ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Private aArea1	:= Getarea()
...
cQuery += ", SB1.B1_XFAMILI FAMILIA"   // 02
cQuery += ", SB1.B1_TIPO TIPO"         // 03
cQuery += ", SB1.B1_GRUPO GRUPO"       // 04
cQuery += ", SB1.B1_DESC DESCRI"       // 05
cQuery += ", SB1.B1_UM UM"             // 06
If mv_par18 == 1
	cQuery += ", SB1.B1_SEGUM SEGUM"    // 07
EndIf

If lVEIC
	cQuery += ", SB1.B1_CODITE CODITE" // 08
EndIf
...
If FWModeAccess("SB1",3) == "E"
	cQuery +=  " SB1.B1_FILIAL = SB2.B2_FILIAL AND "
Else
...
cQuery += " GROUP BY"
If ! lVEIC
	cQuery += " SB2.B2_COD, SB1.B1_XFAMILI,SB1.B1_TIPO, SB1.B1_GRUPO"
Else	
...
Do While !(cAliasTop)->(Eof())
	...
	If dbSeek(XSB1 + (cAliasTop)->COD)
		...
		If (mv_par14 == 1 .OR. ( mv_par14 <> 1 .and. alltrim(str(nQuant,16,2)) <> "0.00")) .And. ;
			...
			If mv_par18==1
					@ Li, 94 + nCOL1 pSay (cAliasTop)->SEGUM
					@ Li, 97 + nCOL1 pSay nQuant2 Picture cPict
