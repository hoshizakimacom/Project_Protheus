...
//+------------------------------------------------------------------------
	//	Relatório de Pesquisa Fábrica / Tipo Movimento
//+------------------------------------------------------------------------
User Function M10R20()
	Local _oReport		:= Nil	
	Local _cPerg  		:= 'M10R20    '
	Local _lOk				:= .F.
...
Static Function ReportDef(_oReport, _cPerg)
	Local _cTitle  	:= 'Relatório de Pesquisa Fábrica / Tipo Movimento'
	Local _cHelp   	:= 'Permite gerar relatório de Pesquisa Fábrica / Tipo Movimento'
	Local _cAlias  	:= GetNextAlias()
	Local _oOP			:= Nil

	_oReport	:= TReport():New('M10R20   ',_cTitle,_cPerg,{|_oReport| ReportPrint(_oReport,_cAlias)},_cHelp)

	//+-------------------------------------------
	//|	Secao dos itens do Pedido de Vendas
	//+-------------------------------------------
	_oOP := TRSection():New(_oReport,'PESQUISA FÁBRICA/TIPO MOVIMENTO',{_cAlias})
	...
	If MV_PAR05 = 1 // Abertas
		BeginSQL Alias _cAlias
			...
			SELECT
					...
					,C6_ITEM
					,C5_NUM
					,MAX(B5_COMPRLC)								AS B5_COMPRLC
					,MAX(B5_LARGLC)									AS B5_LARGLC
					,MAX(B5_ALTURLC) 								AS B5_ALTURLC
					,CK_XITEMP												
					,C2_ITEM										AS ITEM
			...
			LEFT JOIN %Table:SCK% SCK
					...
					AND C6_ITEM    = CK_ITEM
			LEFT JOIN %Table:SD3% SD3								// #7628	
					ON SD3.%NotDel%									// #7628	
					AND CONCAT(C2_NUM,C2_ITEM,C2_SEQUEN) = D3_OP	// #7628
