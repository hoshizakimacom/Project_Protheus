...
If AllTrim(cCodUser) == "000508" .Or.  AllTrim(cCodUser) == "000014" 
	cCodUser  := mv_par03
EndIf
...
(CT2_DEBITO BETWEEN AKX_XCTADE AND AKX_XCTAAT OR CT2_DEBITO = LEFT(AKX_XCTCON,20)) AND
CT2_DEBITO <> LEFT(AKX_XCTNCO,20) AND
CT2_DEBITO <> ' ' AND
CT2.%NotDel% AND 
...
WHERE CT2_FILIAL <> ' ' AND 
CT2_DATA BETWEEN %Exp:cDtIniRef% AND %Exp:cDtFimRef% AND
(CT2_CREDIT BETWEEN AKX_XCTADE AND AKX_XCTAAT OR CT2_CREDIT = LEFT(AKX_XCTCON,20)) AND
CT2_CREDIT <> LEFT(AKX_XCTNCO,20) AND
CT2_CREDIT <> ' ' AND 
CT2.%NotDel%  AND 
AKX_USER = %Exp:cCodUser%
...
While !oReport:Cancel() .And. !(cAliasQry)->(Eof())
	...
	cDescItD  := (cAliasQry)->ITEM_CONT_DESC
