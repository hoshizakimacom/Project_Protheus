...
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|| Programa  | M07R03       |Autor |Valdemir Miranda                | Data | 18/04/2023||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|| Desc.     | Geração de Relatório de Relação Salarial                                ||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
*/
User Function M07R03()

Private lEnd      := .F.
Private nLastKey  := 0
Private lPerg     := lTemreg :=.f.
Private cAliasSRG :=""
Private cPlanilha := "Funcionários"  // *** Nome da Pasta que sera' criada no arquivo Excel               *** //
Private cTabela   := "Relacao Salarial "  // *** Nome do Titulo do Relatorio que sera' criado no arquivo Excel *** //
...
// ...
lParametro:=Pergunte("M07R0003",.T.)
...
Processa({||RELM07R03()}, "Relacao Salarial "+time() )   
...
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| Programa  |PARGPER001  |Autor | VALDEMIR MIRANDA  | Data |  12/04/2023  ||
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| Desc.     | Parâmetros da Relação Salarial                              ||
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
*/ 
//Static Function  PARGPER003() 
//Local _sAlias := Alias()
//Local cPerg := PADR("M07R0003",10)
//Local aRegs :={}                       
//Local nX  
//
//lPerg  :=lTemreg :=.t. 
...
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|| Desc.     | Processo de Geração do Relatório de Demitidos               ||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
*/
Static Function RELM07R03()
...
// *** Acessa função de Captura de Informações da Tabela de Funcionários Demitidos *** //
MQrySRAR03()
...
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
*/
Static Function MQrySRAR03()
Local cAtualiza:= "S"

// ***
cQuery := "SELECT *, SRA.RA_FILIAL,SRA.RA_MAT,SRA.RA_NOME,SRA.RA_ADMISSA,SRA.RA_CC,SRA.RA_SITFOLH,SRA.RA_CODFUNC,SRA.RA_SITFOLH,SRA.RA_TNOTRAB,SRA.RA_CATFUNC,SRA.RA_SALARIO,CTT.CTT_CUSTO, "
cQuery += " CTT.CTT_DESC01,SRJ.RJ_FUNCAO,SRJ.RJ_DESC,SR6.R6_TURNO,SR6.R6_DESC "
...
Do While .not. eof()
   ...
   if cAtualiza = "S"
      ...
      IncProc("Documento: " + (cAliasSRA)->RA_FILIAL+"-"+cFilName+" - "+(cAliasSRA)->RA_MAT+" - "+cRANOME )
  
      // ***
      aadd(aLinha,{(cAliasSRA)->RA_FILIAL,cFilName,(cAliasSRA)->RA_MAT,cRANOME,cADemissax,cRACC,cDESCcc,(cAliasSRA)->RA_CODFUNC,(cAliasSRA)->RJ_DESC,;
      (cAliasSRA)->R6_TURNO,(cAliasSRA)->R6_DESC, (cAliasSRA)->RA_SALARIO })
   endif
    // ***
   dbSelectArea(cAliasSRA) 
   DBSKIP()
