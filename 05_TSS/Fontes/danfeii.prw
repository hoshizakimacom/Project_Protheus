...
For nZ := 1 To nLenDet
	...
	// Tratamento para impressão de códifo FINAME
    If SC5->C5_XFINAME == "1"
    	cFiname 					:= Posicione("SB1",1,xFilial("SB1")+oDet[nX]:_Prod:_cProd:TEXT,"B1_XFINAME")

    	If Len(oDet[nX]:_NITEM:TEXT) == 1
    		cSerMacom					:= Posicione("ZA0",1,xFilial("ZA0")+SC5->C5_NUM + "0" + oDet[nX]:_NITEM:TEXT,"ZA0_SERIE")
    	Else
    		cSerMacom					:= Posicione("ZA0",1,xFilial("ZA0")+SC5->C5_NUM+oDet[nX]:_NITEM:TEXT,"ZA0_SERIE")	
    	Endif
    	
    	oDet[nX]:_PROD:_XPROD:TEXT	:= "Cod.Finame:" + cFiname + " | " + "Nr.Serie:" + cSerMacom + " | " + oDet[nX]:_PROD:_XPROD:TEXT
    Endif
...
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| aCabec     Array contendo as strings de cabeçalho das colunas					||
//|| aValores   Array contendo os valores que serão populados nas colunas.			||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
//|| aTamCol  Array contendo os tamanhos das colunas baseados nos valores.			||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
///|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| RetTamTex-Retorna o tamanho em pixels de uma string. (Workaround para o GetTextWidth)  ||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| cTexto   Texto a ser medido.															||
//|| oFont    Objeto instanciado da fonte a ser utilizada.									||
//|| oPrinter Objeto de impressão instanciado.												||
//|| nTamanho Tamanho em pixels da string.													||
///|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
///|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| PosQuebrVal -Retorna a posição onde um valor deve ser quebrado                         ||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| cTexto Texto a ser medido.																||
//|| nPos   Posição aonde o valor deve ser quebrado.										||
///|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
///|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| executeRetorna - Executa o retorna de notas					                        ||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| cID ID da nota que sera retornado														||
//|| aRetorno   Array com os dados da nota													||
///|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| SimpDanfe - Impressao do formulario DANFE grafico conforme laytout no formato simplificado. ||
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| oDanfe		Objeto FWMSPrinter, para desenho do documento.									 ||
//|| oNfe		Objeto do XML da NF-e															 ||
//|| cCodAutSef	Código de autorização da NF-e													 ||
//|| cModalidade	Modalidade de transmissão da NF-e											 ||
//|| oNfeDPEC	Objeto do XML do DPEC.															 ||
//|| cCodAutDPEC	Codigo de autorização do DPEC												 ||
//|| cDtHrRecCab	Hora de Recebimento da NF-e													 ||
//|| dDtReceb	Data de recebimento da NF-e														 ||
//|| aNota		Array com informações do documento a ser impresso.								 ||
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
Private nFimLUlt		:= 825 //NUMERO DA LINHA FINAL SE FOR ULTIMA PAGINA
Private aMensagem   := {}
Private lImpAnfav   := .F.//GetNewPar("MV_IMPANF",.F.)
Private lImpInfAd   := .F.//GetNewPar("MV_IMPADIC",.F.)      
...
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| ColDadosNf - Devolve os dados com a informação desejada conforme parâmetro nInf.								  ||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| nInf, inteiro, Codigo da informação desejada:<br>1 - Normal<br>2 - Cancelametno<br>3 - InutilizaçãonInf, inteiro,|| 
//||       Codigo da informação desejada:<br>1 - Normal<br>2 - Cancelametno<br>3 - Inutilização	     				  ||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//|| aRetorno Array com as posições do XML desejado, sempre deve retornar a mesma quantidade de posições.			  ||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
